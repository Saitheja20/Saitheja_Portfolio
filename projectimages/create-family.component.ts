...
import { FamilyViewService } from 'src/app/services/family-view/family.view.service';
...
export class CreateFamilyComponent {
  ...
  constructor(private router:Router, private route: ActivatedRoute, private http:HttpClient,
    ...
    this.CreateAccountForm = this.fb.group({
      ...
      zipCodecity: new FormControl('',[Validators.required]),
      sta: new FormControl('',[Validators.required, Validators.pattern(/^([0-9]{5})(?:[-\s]*([0-9]{4}))?$/)]),
      ...
      // risingSchoolGradeDescription:new FormControl("",[Validators.required]),
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.route.queryParams
      .subscribe(params => {
        console.log(params);
        let urlLength = this.router.url.split('/');
        let persID = parseInt(urlLength[urlLength.length-1]);
        this.personUpdateData  = !Number.isNaN(persID)? persID : undefined;
        this.action=this.personUpdateData ? 'Edit':'Add';
      }
    );
    this.dataReceived=JSON.parse(localStorage.getItem('profileData')||'');

    // this.arrayTime1 = this.CreateAccountForm.get('relationList');
    this.fetchpersonTypeList();
    this.fetchstateList();
    this.fetchstatus();
    this.fetchmaritalStatus();
    this.fetchgender();
    this.fetchRelationshipPrimaryContactList();
    this.fetchParentDataList();
    this.fetchAdultPersonData();
    this.fetchCustodyListData();
    this.fetchYesorNoList();
  ...
  fetchpersonTypeList(){
    this.authService.personTypeList().subscribe(response => {
  ...
  stateList:any;
  fetchstateList(){
    this.authService.stateList().subscribe(response => {
  ...
  fetchmaritalStatus(){
    this.authService.maritalStatus().subscribe(response => {
  ...
  fetchGenderList:any;
  fetchgender(){
    this.authService.genderList().subscribe(response => {
      this.fetchGenderList = response;
  ...
  fetchRelationshipPrimaryContactList(){
    this.authService.fetchRelationshipPrimaryContactList().subscribe(response => {
      this.fetchPrimaryContactList = response;
  ...
  fetchCustodyListData(){
    this.authService.fetchCustodyList().subscribe(response => {
      this.fetchCustodyData = response;
      console.log("============================================ Custody Data ================================================")
      console.log(JSON.stringify(response,null,4));
    },(err : HttpErrorResponse)=>{
      var msg = err.error;
      alert(msg.message);
  ...
  fetchYesorNO:any;
  fetchYesorNoList(){
    this.authService.fetchYesorNo().subscribe(response => {
      this.fetchYesorNO = response;
  ...
  fetchParentList:any;
  fetchParentDataList(){
    this.authService.fetchparentList().subscribe(response => {
      this.fetchParentList = response;
  ...
  fetchAdultPersonData(){
    ...
    this.authService.fetchAdultPersonList(param).subscribe(response => {
      this.fetchAdultPersons = response;
  ...
  async fetchUpdatePerson(){
    ...
    let response:any = await this.authService.FetchUpdatePersonData(param);
     ...
     this.CreateAccountForm.controls['schoolGrade'].setValue((response.schoolGradeCode!=undefined && response.schoolGradeCode!=null)?response.schoolGradeCode:'');
    ...
    AddFamliyMemberDatapopulate(){
      ...
      let  response = JSON.parse(localStorage.getItem('CurrentUser') || '');
      this.fetchPersonUpdateResponse = response;
      this.primaryContact='';
      this.personTypeVal=response.personType;
      this.CreateAccountForm.controls['lastName'].setValue(response.lastName);
 ...
 uploadFile(file:any,personId:any){
  return new Promise((resolve,reject)=>{
    ...
    this.authService.upload('/file/uploadFile',file, queryParam).subscribe((event: any) => {
 ...
 downloadFile(eve:any){
  ...
  this.authService.downloadUploadFile(queryParam).subscribe(response => {
 ...
 saveFamilyPerson(type:any){
  ...
  if(this.validateRquired()){
    ...
    Object.assign(param.user, this.CreateAccountForm.getRawValue());
      ...
      this.authService.savePersonFamily(param,  APIName).subscribe(async response => {
 ...
 fetchSchoolGradeList(){
  ...
  this.programService.fetchSchoolGradeList().subscribe((data:any)=>{
    this.CreateAccountForm.controls['schoolGrade'].setValue("");
      this.schoolGradesList = data['selectDropdownList'];
      resolve("");
    })
  })
}
...
schGradeLabel:any;
 async fetchSchooldGradeLabel(){
  ...
  return new Promise((resolve,reject)=>{
    this.programService.fetchSchoolGradeLabel(param).subscribe((data:any)=>{
...
raisingSchooldGradeLabel(){
  let param = {
    code: this.chapterCode,
    programCategory: ''
  }
  return new Promise((resolve,reject)=>{
    this.programService.fetchRisingGradeLabel(param).subscribe((data:any)=>{
